...
const SportsGroupPage: React.FC = () => {
  const router = useRouter();
  const { user, isLoading: authLoading } = useAuth();
  const [records, setRecords] = useState<SportsGroupStaging[]>([]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<SportsGroupStaging | null>(null);
  const [isViewModalOpen, setIsViewModalOpen] = useState(false);
  const [loadingDetail, setLoadingDetail] = useState(false);

  // Fetch sports group records
  useEffect(() => {
    if (!authLoading && user) {
      fetchRecords(1);
    }
  ...
  const fetchRecords = async (page: number) => {
    try {
      ...
      const response = await fetch(
        `/api/staging/sports-group?page=${page}&limit=${pagination.limit}`
      );
  ...
  const handleViewDetail = async (record: SportsGroupStaging) => {
    setSelectedRecord(record);
    setLoadingDetail(true);
    try {
      const response = await fetch(`/api/staging/sports-group/${record.id}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleEdit = (id: number) => {
    router.push(`/admin/data-keolahragaan/sports-group/${id}/edit`);
  };
  ...
  return (
    <div className="p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Data Grup Olahraga Input</h1>
        <p className="text-gray-600 mt-1">Kelola data grup olahraga yang Anda submit untuk verifikasi</p>
      </div>

      {/* Header dengan tombol tambah */}
      <div className="flex justify-end mb-6">
        <button
          onClick={() => router.push('/admin/data-keolahragaan/sports-group/create')}
          className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium"
        >
          <Plus className="h-4 w-4" />
          Tambah Grup Olahraga
        </button>
      </div>
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-gray-900">Detail Grup Olahraga</h2>
                <button
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-gray-900">{selectedRecord.groupName}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Ketua</label>
                      <p className="mt-1 text-sm text-gray-900">{selectedRecord.leaderName || '-'}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Jumlah Anggota</label>
                      <p className="mt-1 text-sm text-gray-900">{selectedRecord.memberCount}</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Cabang Olahraga</label>
                      <p className="mt-1 text-sm text-gray-900">{selectedRecord.sportName || '-'}</p>
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-900">{selectedRecord.decreeNumber || '-'}</p>
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700">Alamat Sekretariat</label>
                    <p className="mt-1 text-sm text-gray-900">{selectedRecord.secretariatAddress || '-'}</p>
                  </div>
...
export default SportsGroupPage;