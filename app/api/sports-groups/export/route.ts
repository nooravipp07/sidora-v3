...
export async function GET(request: NextRequest) {
    try {
        ...
        const excelData = await SportsGroupService.exportToExcel({
            kecamatanId,
            desaKelurahanId,
            year,
            isVerified
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Data Kelompok Olahraga');

        // Set column widths
        const columns = [
            'No', 'Tahun', 'Cabang Olahraga', 'Nama Klub/Kelompok', 'Nama Ketua',
            'Jumlah Anggota', 'Verifikasi', 'Nomor SK', 'Alamat Sekretariat',
            'Desa/Kelurahan', 'Kecamatan', 'Tanggal Dibuat', 'Tanggal Diperbarui'
        ];
        ...
        const filename = `KelompokOlahraga_${new Date().getTime()}.xlsx`;
        ...
        console.error('Error exporting sports group data:', error);
        return NextResponse.json(
